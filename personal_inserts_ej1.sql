...
select nombre , sal_emp as salario_maximo from empleados where sal_emp = (select max(sal_emp) from empleados);
...
select nombre_depto from departamentos group by nombre_depto having nombre_depto=0;
-- Mostrar la lista de empleados, con su respectivo departamento y el jefe de cada departamento;
select e.nombre as empleado, d.nombre_depto as departamento, d.nombre_jefe_depto as jefe
from empleados e right join departamentos d on e.id_depto= d.id_depto;
-- Mostrar la lista de los empleados cuyo salario es mayor o igual que el promedio de la;
-- empresa. Ordenarlo por departamento;
select e.nombre as empleado, e.sal_emp as salario, d.nombre_depto as departamento
